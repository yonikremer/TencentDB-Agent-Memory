...
export function registerKnowledgeCodeGraphRoutes(
  api: Hono,
  deps: PanelDeps,
): void {
  ...
  api.post("/knowledge/code-graph/create", mw, async (c) => {
    ...
    try {
      ...
      if (ctx.userKey) {
        ...
        deps.logger.info(
          "[code-graph/create] stashed owner key for S2S meta register",
          {
            knowledge_id: detail.code_graph_id,
            team_id: teamId,
            owner: gate.userId,
        ...
        deps.logger.warn(
          "[code-graph/create] no user_key in ctx; cannot stash for S2S register",
          {
            knowledge_id: detail.code_graph_id,
            team_id: teamId,
          },
  ...
  api.post("/knowledge/code-graph/register-meta", mw, async (c) => {
    ...
    log.info("[code-graph/register-meta] invoked (frontend fallback)", {
      code_graph_id: cgId,
      team_id: teamId,
    });
    const gate = await requireTeamMember(deps, c, ctx, teamId);
    ...
    if (detail.status !== "ready") {
      log.warn("[code-graph/register-meta] not ready → 409", {
        code_graph_id: cgId,
    ...
    if (detail.owner_user_id && detail.owner_user_id !== gate.userId) {
      log.warn("[code-graph/register-meta] owner mismatch → 403", {
        code_graph_id: cgId,
        ks_owner: detail.owner_user_id,
        caller: gate.userId,
      });
      return respondControlError(c, 403, "NOT_RESOURCE_OWNER");
    }
    log.info(
      "[code-graph/register-meta] gating passed; registering meta asset",
      {
        code_graph_id: cgId,
        owner: gate.userId,
      },
    ...
    return respondEnvelope(
      c,
      okEnvelope(c, { registered: true, code_graph_id: cgId }),
    );
  });
  ...
  api.post("/knowledge/code-graph/delete", mw, async (c) => {
    ...
    if (cgIds.length === 0)
      return respondControlError(c, 400, "MISSING_CODE_GRAPH_ID");
