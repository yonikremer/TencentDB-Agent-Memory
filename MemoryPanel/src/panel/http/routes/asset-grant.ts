...
export function registerAssetGrantRoutes(api: Hono, deps: PanelDeps): void {
   ...
   api.post("/asset/grant", mw, async (c) => {
      ...
      if (!(await resolveCallerUserId(deps, ctx)))
         return respondControlError(c, 401, "UNAUTHORIZED");
      ...
      if (!assetId || !nodeId)
         return respondControlError(c, 400, "MISSING_PARAM");
      if (action !== "grant" && action !== "revoke")
         return respondControlError(c, 400, "BAD_ACTION");
      if (!(GRANT_TYPES as readonly string[]).includes(grantType))
         return respondControlError(c, 400, "BAD_GRANT_TYPE");
      ...
      const grantEnv = await callKernel("groupy/asset-grant", {
         asset_id: assetId,
         node_id: nodeId,
         action,
