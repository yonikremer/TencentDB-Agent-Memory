...
export function registerGroupyRoutes(api: Hono, deps: PanelDeps): void {
  ...
  api.post("/groupy/orphans", mw, async (c) => {
    ...
    const tree = (treeEnv.data ?? { nodes: [] }) as {
      nodes: Array<{ node_id: string; archived: boolean }>;
    };
    const archived = new Set(
      tree.nodes.filter((n) => n.archived).map((n) => n.node_id),
    );
  ...
  api.post("/groupy/mirror-sync", mw, async (c) => {
    ...
    for (const share of shares) {
      ...
      let assetEnv;
      try {
        assetEnv = await deps.metaKernel.invoke(
          "asset/get",
          { asset_id: share.asset_id },
          ctx,
        );
      ...
      try {
        ...
        report.push({
          ...
          set: 0,
          cleared: 0,
          skipped: "KS unreachable",
        });
        continue;
      }
      const have = new Map(
        (current.grants ?? []).map((g) => [g.team_id, g.grant_type]),
      );
      const missing = [...desired.entries()].filter(
        ([t, gt]) => have.get(t) !== gt,
      );
