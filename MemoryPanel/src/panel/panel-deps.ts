...
export function buildPanelDeps(config: PanelConfig): PanelDeps {
  ...
  const instanceRegistry = InstanceRegistry.load(
    config.metadataInstancesConfig,
  );
  const kernelHttp = new FetchKernelHttpAdapter(logger);
  const metaKernel = new FetchMetaKernelAdapter(
    kernelHttp,
    config.metadataRemoteTimeoutMs,
  );
  const knowledgeClientFactory = (
    instanceId: string,
    userKey?: string,
  ): KnowledgeClientPort =>
  ...
  const skillKernel = new FetchSkillKernelAdapter(
    kernelHttp,
    config.metadataRemoteTimeoutMs,
  );
