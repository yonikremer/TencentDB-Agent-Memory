#!/usr/bin/env bash
# install-git-hooks.sh — 在本仓库启用 secret-leak-check pre-commit hook
#
# 用法：bash scripts/install-git-hooks.sh
#
# 效果：每次 `git commit` 前自动跑 scripts/secret-leak-check.sh，命中即拒绝提交。
...
echo "  每次 git commit 前会跑 secret-leak-check.sh；命中敏感信息即拒绝提交。"
