...
const fakeKs = {
  ...
  async codeGraphGet(id: string) {
    ...
    if (ksDown) throw new Error("ks down");
    if (id !== FAKE_CG_BASE.code_graph_id)
      throw new DomainError("cg not found", "NOT_FOUND", 404);
    return { ...FAKE_CG_BASE, status: ksStatus };
  },
  async codeGraphSync() {
...
describe("code-graph create + in-flight owner read", () => {
  it("missing repo_url -> 400", async () => {
    const r = await post("/api/v1/knowledge/code-graph/create", {
      team_id: TEAM,
    });
  ...
  it("create stashes owner task for S2S register", async () => {
    ...
    expect(taskRecords[0]).toMatchObject({
      knowledge_id: "cg-fake0001",
      owner_user_id: USER_ID,
    });
  });

  it("get readable by KS owner before meta registered", async () => {
    const r = await post("/api/v1/knowledge/code-graph/get", {
      code_graph_id: "cg-fake0001",
    });
    expect(r.json.code).toBe(0);
  });
...
describe("register-meta gating", () => {
  it("not ready -> 409 CODE_GRAPH_NOT_READY", async () => {
    ksStatus = "processing";
    try {
      const r = await post("/api/v1/knowledge/code-graph/register-meta", {
        team_id: TEAM,
        code_graph_id: "cg-fake0001",
      });
  ...
  it("ready -> registers meta asset", async () => {
    ...
    expect(assets.get("cg-fake0001")).toMatchObject({
...
describe("sync / query / delete / failure mapping", () => {
  it("sync passes through to KS", async () => {
    ksSeen.length = 0;
    const r = await post("/api/v1/knowledge/code-graph/sync", {
      code_graph_id: "cg-fake0001",
    });
    expect(r.json.code).toBe(0);
  ...
  it("search returns KS text block", async () => {
    const r = await post("/api/v1/knowledge/code-graph/search", {
      code_graph_id: "cg-fake0001",
      query: "auth",
    });
    expect(r.json.code).toBe(0);
  ...
  it("delete cascades meta asset", async () => {
    const r = await post("/api/v1/knowledge/code-graph/delete", {
      code_graph_ids: ["cg-fake0001"],
    });
    expect(r.json.code).toBe(0);
  ...
  it("KS down on registered asset -> 502 (unregistered masks as 404 via owner fallback)", async () => {
    await post("/api/v1/knowledge/code-graph/register-meta", {
      team_id: TEAM,
      code_graph_id: "cg-fake0001",
    });
    ksDown = true;
