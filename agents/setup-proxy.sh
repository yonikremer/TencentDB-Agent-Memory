...
AGENT_LABELS=(
  ...
  "Hermes            — OpenAI Chat + Header Preselection, ~/.hermes/config.yaml"
  "OpenClaw          — OpenAI Chat + Header Preselection, ~/.openclaw/openclaw.json"
)
...
if [[ -n "$ARG_AGENT" ]]; then
  ...
  select_one "Which Agent should be configured this time?" "${AGENT_LABELS[@]}"
fi
...
if needs_header_preselect "$CHOSEN_AGENT"; then
  ...
  info "${CHOSEN_AGENT} does not support interactive Form, needs to pre-fill team/agent/task ID in the configuration file"
  echo ""

  # Ask if user wants to provide Panel address for auto-discovery
  USE_PANEL_API=false
  if confirm "Whether to provide the panel backend address to automatically fetch the Team/Agent/Task list?" "y"; then
    ...
    if [[ "$PANEL_HTTP" == "000" || "$PANEL_HTTP" =~ ^5 ]]; then
      warn "Panel backend is unreachable (HTTP ${PANEL_HTTP}), will switch to manual input"
    else
  ...
  if $USE_PANEL_API; then
    ...
    if [[ "$TEAM_COUNT" -eq 0 ]]; then
      warn "Team not found (possibly insufficient Key permissions or Team not created), change to manual input"
      USE_PANEL_API=false
    else
  ...
  success "Header pre-selection complete"
else
...
EOF
  ...
  echo ""
  info "How to start:"
  echo -e "  ${GREEN}claude${RESET}   ${DIM}# Start directly, reads env from settings.json${RESET}"
...
case "$CHOSEN_AGENT" in
  ...
  codex)
    ...
    ;;
  workbuddy)
    warn "You need to switch to the custom model ${BOLD}${MODEL_ID}${RESET} in the WorkBuddy model selector when using it, otherwise it will go through Proxy"
    ;;
  dsh)
    warn "Just run dsh directly when using, settings.yaml has specified the model"
    ;;
  hermes|openclaw)
...
echo ""
info "An 'Team→Agent→Task' selection will be prompted on first use (when ${CHOSEN_AGENT} supports interactive Form)"
...
if [[ -f "$ASSET_IMPORT_SCRIPT" ]] && ! $ARG_NONINTERACTIVE; then
  ...
  if confirm "Import this Agent's local assets (skill + conversation) to team memory?" "n"; then
    ...
    if [[ -z "$RUNNER" ]]; then
      warn "tsx or npx not found, cannot directly run the asset import script"
