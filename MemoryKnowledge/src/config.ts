 ...
 */
export function getIngestConcurrency(
 ...
 */
export function getGlobalLlmConcurrency(
 env: NodeJS.ProcessEnv = process.env,
): number {
 ...
 */
export function getWikiRetrievalEnabled(
 env: NodeJS.ProcessEnv = process.env,
): boolean {
 ...
 */
export function getWikiRetrievalTopK(
 env: NodeJS.ProcessEnv = process.env,
): number {
 ...
 */
export function getWikiRetrievalMaxChars(
 env: NodeJS.ProcessEnv = process.env,
): number {
 ...
 */
export function getWikiRetrievalQueryTerms(
 env: NodeJS.ProcessEnv = process.env,
): number {
...
function validateClickHouseConfig(config: ClickHouseTelemetryConfig): void {
 ...
 if (!identifier.test(config.database))
  throw new Error("Invalid KNOWLEDGE_CLICKHOUSE_DATABASE");
 if (!identifier.test(config.table))
  throw new Error("Invalid KNOWLEDGE_CLICKHOUSE_TABLE");
 if (config.flushIntervalMs < 100)
  throw new Error("KNOWLEDGE_CLICKHOUSE_FLUSH_INTERVAL_MS must be >= 100");
 if (config.flushThreshold < 1)
  throw new Error("KNOWLEDGE_CLICKHOUSE_FLUSH_THRESHOLD must be >= 1");
 if (config.ttlDays < 0)
  throw new Error("KNOWLEDGE_CLICKHOUSE_TTL_DAYS must be >= 0");
 if (config.requestTimeoutMs < 100)
  throw new Error("KNOWLEDGE_CLICKHOUSE_REQUEST_TIMEOUT_MS must be >= 100");
}
...
export function loadConfig(): ServiceConfig {
 ...
 return {
  ...
  llm: {
   ...
   protocol:
    env("LLM_PROTOCOL", "openai") === "anthropic" ? "anthropic" : "openai",
