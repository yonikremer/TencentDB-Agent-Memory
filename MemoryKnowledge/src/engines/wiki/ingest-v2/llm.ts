...
import { recordLlmError } from "../../../dependency-health.js";
...
export function normalizeLlmConfig(
      ...
      const maxTokens =
            cfg.maxTokens ?? cfg.maxContextSize ?? DEFAULT_MAX_TOKENS;
...
export function createLlmClient(config: NormalizedLlmConfig): LlmClient {
      ...
      if (!config.baseUrl || !config.baseUrl.trim()) {
            throw new Error(
                  "LLM endpoint not configured (no usable llm_binding and LLM_MODE is not custom). " +
                        ...
                        "(needs KNOWLEDGE_AUTH_TOKEN service bearer or a valid user key), " +
      ...
      return {
            ...
            async chat(params: ChatParams): Promise<string> {
                  ...
                  log.debug(
                        `LLM system prompt [${label}] (model=${config.model})`,
                        {
                              text: params.system.slice(0, 200),
                        },
                  );
                  log.debug(
                        `LLM user prompt [${label}] (model=${config.model})`,
                        {
