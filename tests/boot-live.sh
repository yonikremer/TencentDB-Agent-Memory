...
[ "${BOOT_LIVE:-0}" = "1" ] || { echo "SKIP: set BOOT_LIVE=1 to run live boot"; exit 0; }
[ "${LLM_LIVE:-0}" = "1" ] || { echo "SKIP: set LLM_LIVE=1 to run live boot"; exit 0; }
...
[ -n "$OR_KEY" ] || { echo "FAIL: no OPENROUTER_API_KEY in tests/.env"; exit 1; }
...
for url in "http://127.0.0.1:$CORE_P/health" "http://127.0.0.1:$HUB_P/health" "http://127.0.0.1:$PROXY_P/health"; do
  if curl -fs -m 10 "$url" >/dev/null; then echo "PASS healthy $url";
  else echo "FAIL unhealthy $url"; ENV_FILE="$TEST_ENV" "$G/stop-all.sh" || true; exit 1; fi
done
...
[ "$left" -eq 0 ] || { echo "FAIL: $left tdai containers remain"; exit 1; }
echo "== live boot PASS =="
