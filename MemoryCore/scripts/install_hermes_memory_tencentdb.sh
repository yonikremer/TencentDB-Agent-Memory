...
#
# Path conventions (all located under ~/.memory-tencentdb/, can be overridden via environment variables):
...
#
# Note: When root directly ssh logs in and runs (not via sudo), neither of the first two will be set,
# ``whoami`` returns ``root``. The ``id -u`` == 0 branch below recognizes this "the target
# is root" situation and skips the ``su - root`` recursion.
...
# Old path (for automatic migration only)
...
# logging in as root to run the installation), skip ``su - root`` -- otherwise it will infinitely recurse (
# ``su - root`` enters root, then reaches this branch again, su again, and never stops). See issue #20.
...
if [ "$(id -u)" -eq 0 ] && [ "$USERNAME" != "root" ]; then
    ...
    # Currently root and the target user is also root: directly run the subsequent installation logic as root,
...
# ---------- Step 1: Download packages via npm and extract them to $TDAI_INSTALL_DIR ----------
...
# Move package contents to target installation directory
...
# Install tsx (required for Gateway startup), prefer installing locally
...
# ---------- Step 3: Prompt user to manually enable memory_tencentdb (do not automatically modify config) ----------

echo "[memory-tencentdb] Step 3: Checking hermes config..."

# The plugin has been linked to the hermes plugin directory, but is not enabled by default, only prompting
