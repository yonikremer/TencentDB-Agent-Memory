/**
 * v3 metadata API request body Zod schemas (55 public APIs).
 *
 * Corresponds to design doc §7.1. Each schema validates the request body for the corresponding route;
 * route handlers use `schema.safeParse(body)` to validate before calling MetadataService.
 */
import { z } from "zod";
...
/** Either user_id or user_key (for compatibility with old callers passing only user_id). */
...
export const userCreateSchema = z.object({
  username: nonEmpty,
  // Optional: when creating "service accounts" from management/internal side, specify a deterministic user_id (e.g. knowledge-service),
  // making it easy to hit proxy systemUsers whitelist by stable user_id; if not passed, kernel generates random usr-xxx.
  // Only system_admin can call this API (see v3-meta-router assertCanManageUsers).
  user_id: z.string().min(1).optional(),
});

// Sister API to /v3/meta/user/create: allows system_admin to explicitly specify user_key when creating account.
// user_id is not accepted as input (zod strips it by default), it's generated by kernel and returned; user_key format is caller's responsibility,
// kernel only does non-empty validation + DB layer UNIQUE fallback (duplicate throws duplicate_user_key).
...
export const teamUpdateSchema = z.object({
  ...
  description: z.string().optional(),
  // owner_user_id cannot be changed: input is silently ignored by zod default strip
  status: teamStatus.optional(),
  metadata_json: z.string().optional(),
});
...
export const fixedAssetListWithDetailSchema = z.object({
  ...
  /**
   * Optional type filter: only return bindings where asset_type is in this list.
   * Empty array or omitted = no filter (returns all types).
   * Used when caller explicitly only needs specific asset types (e.g. proxy knowledge-tools-injector
   * only needs llm_wiki + code_graph), avoiding truncation of types outside the pagination page in a mixed list.
   */
  asset_types: z.array(assetType).optional(),
}).merge(paginationInputSchema);

/** deduplicate agent_ids; 1-100. */
...
export const assetListAccessibleSchema = userIdOrKeyFields
  .extend({
    ...
    asset_type: assetType.optional(),
    agent_id: z.string().optional(),
    // Optional server-side visibility filter:
    //   - Single value: `visibility: "team"` → only return team visible (used for management page "team assets" tab)
    //   - Array: `visibility: ["team", "restricted"]` → whitelist approach
    //   - Omitted: no visibility filtering (returns all accessible, including own private)
    // Key purpose: prevent frontend "team assets" tab from getting own private data at HTTP layer,
    // avoiding information leakage risks of "response body contains all data, frontend JS filters it".
    visibility: z.union([visibility, z.array(visibility).min(1).max(5)]).optional(),
  })
  .merge(paginationInputSchema)
  .refine(requireUserIdOrKey, userIdOrKeyRefine);

/** internal list-by-instance: deduplicate user_ids; omitted or empty array → undefined (no filter). */
...
/** internal: instance user list (not written to public API docs). */
