...
export interface ManifestStoreInfo {
 ...
 /** @deprecated tcvdb removed; kept for reading old manifests. */
 tcvdb?: {
  url: string;
  database: string;
  alias?: string;
...
export interface StoreConfigSnapshot {
 type: "sqlite";
 sqlitePath?: string;
 /** @deprecated ignored; kept for compat. */
 tcvdbUrl?: string;
 /** @deprecated ignored; kept for compat. */
...
export function diffStoreBinding(
 ...
 if (persisted.type === "sqlite" && current.type === "sqlite") {
  if (persisted.sqlite?.path !== current.sqlite?.path) {
   diffs.push(
    `sqlite path changed: ${persisted.sqlite?.path} → ${current.sqlite?.path}`,
   );
  }
 }

 if (
  (persisted as ManifestStoreInfo).tcvdb ||
  (current as ManifestStoreInfo).tcvdb
 ) {
  diffs.push("tcvdb entry present in manifest but unsupported (removed)");
 }
