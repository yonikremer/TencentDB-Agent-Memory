 ...
 *
 * This module defines the storage contracts for the sqlite backend.
 *
 * Design principles:
 ...
 * 4. **Sync-first**: Matches SQLite DatabaseSync usage.
 */
...
export interface IMemoryStore
 extends MemoryPromptStore,
  MemoryGenerationRefStore {
 // ── Capabilities ───────────────────────────────────────────
  ...
  */
 clearMemoryContent?(
  filter: MemoryContentClearFilter,
 ): MaybePromise<MemoryContentClearResult>;
 ...
 createTeam?(
  input: Omit<
   ...
   | "agent_ids"
   | "task_ids"
  > & { team_id?: string; status?: TeamStatus },
 ): MaybePromise<TeamEntity>;
 ...
 updateTeam?(
  ...
  patch: Partial<
   ...
   >
  >,
 ): MaybePromise<TeamEntity | null>;
 deleteTeams?(teamIds: string[]): MaybePromise<BatchDeleteResult>;

 createUser?(
  input: Pick<UserEntity, "name"> &
   Partial<Pick<UserEntity, "job_description" | "status">> & {
    user_id?: string;
   },
 ): MaybePromise<UserEntity>;
 getUser?(userId: string): MaybePromise<UserEntity | null>;
 updateUser?(
  userId: string,
  patch: Partial<Pick<UserEntity, "name" | "job_description" | "status">>,
 ): MaybePromise<UserEntity | null>;
 ...
 createAgent?(
  ...
  > & { agent_id?: string; status?: AgentStatus; visibility?: AgentVisibility },
 ): MaybePromise<AgentEntity>;
 ...
 updateAgent?(
  ...
  patch: Partial<
   ...
   >
  >,
 ): MaybePromise<AgentEntity | null>;
 deleteAgents?(agentIds: string[]): MaybePromise<BatchDeleteResult>;

 createTask?(
  input: Omit<
   TaskEntity,
   "created_at" | "updated_at" | "source_type" | "agent_ids" | "user_ids"
  > & {
   task_id?: string;
   source_type?: TaskSourceType;
   agent_ids?: string[];
 ...
 updateKnowledge?(
  ...
  patch: Partial<
   ...
   >
  >,
 ): MaybePromise<KnowledgeEntity | null>;
 deleteKnowledge?(
  knowledgeIds: string[],
  teamId?: string,
 ...
 listKnowledge?(input: {
  ...
  limit?: number;
  offset?: number;
 }): MaybePromise<KnowledgeListResult>;
