...
export function recordDependencyError(kind: DependencyKind, message: string): void {
  lastErrors[kind] = { message, at: new Date().toISOString() };
}

/** Last observed error for a dependency, or null when none seen yet. */
export function lastDependencyError(kind: DependencyKind): DependencyErrorRecord | null {
...
export function embeddingDependency(
  ...
  if (!svc) return { configured: false, ready: false, lastError: lastDependencyError("embedding") };
  let ready = false;
  try {
    ready = svc.isReady();
  } catch {
  ...
  return { configured: true, ready, lastError: lastDependencyError("embedding") };
}
