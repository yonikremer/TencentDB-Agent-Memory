...
import {
  memoryGenerationLogGetSchema,
  memoryGenerationLogListSchema,
} from "./memory-generation-log-schemas.js";
...
async function handleList(
  ...
  if (!storage)
    return errorEnvelope(503, "GENERATION_LOG_STORE_UNAVAILABLE", requestId);
  const now = Date.now();
  ...
  try {
    const result = await new MemoryGenerationLogStore(
      storage,
      auth.serviceId,
    ).list({
...
async function handleGet(
  ...
  if (parsed.data.log_id) {
    ...
    return log
      ? successEnvelope(log, requestId)
      : errorEnvelope(404, "MEMORY_GENERATION_LOG_NOT_FOUND", requestId);
  }

  if (!store) return errorEnvelope(503, "Store not available", requestId);
  if (!store.getMemoryGenerationRef)
    return errorEnvelope(
      503,
      "Generation reference store not available",
      requestId,
    );
  const provenance = await store.getMemoryGenerationRef(
    parsed.data.layer!,
  ...
  if (
    !log ||
    !log.output_refs.some((ref) => ref.record_id === parsed.data.memory_id)
  ) {
    return errorEnvelope(404, "MEMORY_GENERATION_LOG_NOT_FOUND", requestId);
  }
...
export function makeMemoryGenerationLogRouteTable(): Record<
  string,
  RouteHandler
> {
  return {
