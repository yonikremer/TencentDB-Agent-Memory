...
注意：`MetadataClient` 只管元数据 CRUD；真正写源文件、ingest、搜 wiki 内容、读页面用 `WikiClient`（Knowledge Service 数据面，`endpoint` 指向 `service_url` 的 `:8421`，一般不需要 `apiKey`）：`wiki.create → rawWrite → ingest → get 轮询到 ready`，分享用 `meta.shareAssetWithTeam(wiki_id / skill_id)`（`asset_id` 与 `wiki_id / skill_id` 对齐）。
