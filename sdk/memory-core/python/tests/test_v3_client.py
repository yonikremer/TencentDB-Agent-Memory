...
def _sync(**kw):
    ...
    args.update(kw)
    stub = FakeStub()
    return MemoryClient(endpoint="http://e", api_key="k", service_id="s", stub=stub, **args), stub  # type: ignore[call-arg]  // FakeStub is runtime-compatible; **args unpacking defeats narrowing.
...
def test_init_variants():
    ...
    real.close()
    with MemoryClient(endpoint="http://e", api_key="k", service_id="s", stub=FakeStub(), **KW) as c:  # type: ignore[call-arg]  // FakeStub is runtime-compatible.
...
async def test_async_mirror():
    ...
    c = AsyncMemoryClient(endpoint="http://e", api_key="k", service_id="s", stub=FakeAsyncStub(), **KW)  # type: ignore[call-arg]  // FakeAsyncStub is runtime-compatible.
    stub = c._stub
    ...
    with pytest.raises(ParamError):
        await AsyncMemoryClient(endpoint="http://e", api_key="k", service_id="s",
                                stub=FakeAsyncStub(), **KW).add_conversation([{"m": 1}])  # type: ignore[call-arg]  // FakeAsyncStub is runtime-compatible.
...
async def test_async_offload_and_read_file(monkeypatch):
    ...
    assert [call[1] for call in stub.calls] == [
        "/v3/offload/ingest", "/v3/offload/compact", "/v3/offload/query-mmd",
    ]
