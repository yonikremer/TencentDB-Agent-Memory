"""TencentDB Agent Memory v3 Python SDK — management plane client (MetadataClient).

Difference from ``v3.client.MemoryClient`` (data plane, strict isolation L0–L3)
------------------------------------------------------------------

- Data plane client requires ``team_id`` / ``agent_id`` / ``user_id`` tuple at construction.
- Management plane client **does not require** isolation tuple; authenticates via Bearer + ``x-tdai-service-id``,
  passing business fields in the request body. Optional ``user_key`` via ``x-tdai-user-key``
  header (required for system_admin endpoints like ``user/create``, ``user/delete``).

Coverage
--------

- ``/v3/meta/*`` public endpoints (54 total, aligned with Panel Control ``META_ACTIONS``)
- ``/v3/knowledge/*`` Knowledge entity CRUD endpoints (5 total)
...
class MetadataClient(_MetadataMethodsMixin):
    """v3 synchronous management plane client — ``/v3/meta/*`` + ``/v3/knowledge/*``."""
...
class AsyncMetadataClient(_MetadataMethodsMixin):
    """v3 asynchronous management plane client — isomorphic with :class:`MetadataClient`, methods are ``async``."""
