...
def _decode_response(resp: httpx.Response) -> dict:
    ...
    try:
        ...
        raise TDAMError(
            resp.status_code if resp.is_error else -1, message, header_request_id
        ) from exc
    ...
    if resp.is_error or code != 0:
        effective_code = (
            code if isinstance(code, int) and code != 0 else resp.status_code
        )
    ...
    if not isinstance(result, dict):
        raise TDAMError(
            -1, "API response data must be a JSON object", header_request_id
        )
